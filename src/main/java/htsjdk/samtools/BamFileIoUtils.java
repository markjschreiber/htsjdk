...
/**
 * Utility class for BAM file I/O operations using Path-based APIs.
 * 
 * <p>This class provides utilities for:
 * <ul>
 *   <li>Reheadering BAM files</li>
 *   <li>Block-level copying of BAM files</li>
 *   <li>Gathering multiple BAM files</li>
 * </ul>
 * 
...
public class BamFileIoUtils {
    ...
    /**
     * Copy a BAM file but replacing the header, with default options to create MD5 and index.
     *
    ...
    /**
     * Gathers multiple BAM files into a single output file using block-level copying.
     * 
     * <p>Assumes that all inputs and outputs are block compressed BAM files and copies them without 
     ...
     *
     * @param bams        List of BAM file paths to gather
     * @param output      Output BAM file path
     ...
     */
    public static void gatherWithBlockCopying(final List<Path> bams, final Path output, final boolean createIndex, final boolean createMd5) {
        try {
            ...
            Path indexFile = null;
            if (createIndex) {
                indexFile = output.resolveSibling(output.getFileName().toString().replaceFirst("\\.[^.]+$", "") + FileExtensions.BAI_INDEX);
            ...
            for (final Path f : bams) {
                LOG.info(String.format("Block copying %s ...", f.toAbsolutePath()));
            ...
            if (createIndex && indexFile != null) {
                try {
                    final long outputModified = Files.getLastModifiedTime(output).toMillis();
                    final long indexModified = Files.getLastModifiedTime(indexFile).toMillis();
                    if (outputModified > indexModified) {
                        Files.setLastModifiedTime(indexFile, Files.getLastModifiedTime(output));
                    }
                } catch (IOException e) {
                    System.err.print(String.format("Index file is older than BAM file for %s and unable to resolve this", output.toAbsolutePath()));
                }
            }
        } catch (final IOException ioe) {
            throw new RuntimeIOException(ioe);
