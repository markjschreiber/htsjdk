...
public class CRAMComplianceTest extends HtsjdkTest {
    ...
    private void roundTripTolerateAmbiguityCodeConversion(final Path inputFile, final Path referenceFile) throws IOException {
        ...
        final Path tempCRAMFile = Files.createTempFile("testAmbiguousBasesBAMThroughCRAMRoundTrip", FileExtensions.CRAM);
    ...
    private void doComplianceTest(
        ...
        cramFileReader = new CRAMFileReader(Files.newInputStream(t.cramFile_21), (SeekableStream) null, source, ValidationStringency.SILENT);
        ...
        cramFileReader = new CRAMFileReader(Files.newInputStream(t.cramFile_30), (SeekableStream) null, source, ValidationStringency.SILENT);
        cramFileReaderIterator = cramFileReader.getIterator();
        for (SAMRecord samRecord : samRecords) {
            Assert.assertTrue(cramFileReaderIterator.hasNext());
            SAMRecord restored = cramFileReaderIterator.next();
